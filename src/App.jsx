...
// Função auxiliar para carregar scripts externos como os dicionários
...
function App() {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      // Função para carregar as Bíblias
      ...
      try {
        ...
        const bibleObject = allBibleData.reduce((acc, versionData) => {
          acc[versionData.id] = versionData.data;
          return acc;
        }, {});
    ...
    <div className="bg-slate-50 min-h-screen">
      <div className="max-w-5xl mx-auto p-4 sm:p-8">
        ...
        <main className="mt-8">
          ...
          {/* A MUDANÇA ESTÁ AQUI: Passando bibleData para o Dicionário */}
          {activeTab === 'dictionary' && <Dictionary greekDict={greekDict} hebrewDict={hebrewDict} bibleData={bibleData} />}
          {activeTab === 'random' && <RandomVerse bibleData={bibleData} />}
        </main>
      </div>
    </div>
  );
